...
def save_image_info(image_path, receive_time):
    save_time = datetime.now()
    sql = "INSERT INTO detected (Datetime, image_path) VALUES (%s, %s)"
    val = (save_time, image_path)
    mycursor.execute(sql, val)
    mydb.commit()
    print(f"Image info saved to database: {save_time}, {image_path}")
    print(f"Time received: {receive_time}, Time saved: {save_time}")
        ...
        for info in result:
            ...
            for box in boxes:
                ...
                if confidence > 50:
                    ...
                    save_image_info(filename, receive_time)
